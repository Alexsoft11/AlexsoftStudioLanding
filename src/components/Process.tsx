  ...
  return (
    <section id="process" className="py-32 px-6 max-w-screen-xl mx-auto">
      <motion.div {...fadeInUp} className="mb-20">
        <h2 className="text-4xl md:text-7xl font-display font-black tracking-[-0.04em] uppercase leading-[1.1] mb-6">
          {t.process.title} <br />
          <span className="text-blue-500 text-3xl md:text-6xl">{t.process.subtitle}</span>
        </h2>
      </motion.div>
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
        {steps.map((step, i) => (
          <motion.div key={i} {...fadeInUp} transition={{ ...fadeInUp.transition, delay: i * 0.1 }} className="group p-8 bg-zinc-900/20 border border-white/5 rounded-3xl hover:bg-zinc-900/40 hover:border-blue-500/30 transition-all">
            ...
            <h3 className="text-xl font-display font-extrabold mb-4 uppercase tracking-tight">{step.title}</h3>
