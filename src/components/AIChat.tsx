...
export const AIChat: React.FC<AIChatProps> = ({ isOpen, onClose, t }) => {
  ...
  const handleSend = async () => {
    ...
    // Add a placeholder for the assistant's response to show we are consulting
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Network error fetching from server');
      }
      ...
      setMessages(prev => {
        ...
        newMessages[newMessages.length - 1] = { role: 'assistant', content: data.text || t.ai.error };
        return newMessages;
      });
    } catch (error) {
      console.error(error);
